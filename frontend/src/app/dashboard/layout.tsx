...
import { AdSidebarDesktop, AdBannerMobile } from "@/components/ads/AdSidebar"
import {
  LayoutDashboard,
  Utensils,
  Sparkles,
...
const NAV = [
  { section: "Wellness", href: "/dashboard", label: "Dashboard", icon: LayoutDashboard },
  { section: "Wellness", href: "/dashboard/foods", label: "Foods", icon: Utensils },
  { section: "Wellness", href: "/dashboard/log", label: "Log meal", icon: Utensils },
  { section: "Wellness", href: "/dashboard/ai", label: "AI Coach", icon: Sparkles },
  { section: "Insights", href: "/dashboard/weekly", label: "Weekly report", icon: TrendingUp },
  { section: "Insights", href: "/dashboard/family", label: "Family plan", icon: Users },
  { section: "Services", href: "/dashboard/experts", label: "Experts", icon: UserCircle },
  { section: "Services", href: "/dashboard/packages", label: "Packages", icon: Package },
  { section: "Services", href: "/dashboard/deals", label: "Kuriftu deals", icon: Bell },
  { section: "Insights", href: "/dashboard/community", label: "Community 🌿", icon: Globe },
]
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      ...
      >
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100">
          ...
          <div className="min-w-0 flex-1">
            <div className="font-bold text-gray-900 text-sm leading-tight">Well-Net</div>
            <div className="text-xs text-wellnet-600 truncate">
              {profile?.display_name || "Wellness Ecosystem"}
            </div>
          </div>
          <button className="lg:hidden shrink-0 p-1" onClick={() => setSidebarOpen(false)}>
            <X className="w-4 h-4 text-gray-400" />
          </button>
        </div>

        {/* Gut Score Stats Widget */}
        {(profile?.current_gut_score ?? 0) > 0 && (
          <div className="mx-4 mt-3 flex items-center gap-3 bg-wellnet-50 rounded-xl px-3 py-2.5">
            ...
            <div>
              <div className="text-xs font-semibold text-wellnet-700">Today's gut score</div>
              ...
              {NAV.filter((n) => n.section === section).map(({ href, label, icon: Icon }) => (
                ...
                >
                  <Icon
                    className={cn(
                      "w-4 h-4 shrink-0",
                      isActive(href) ? "text-wellnet-600" : "text-gray-400"
                    )}
                  />
                  {label}
                </Link>
        ...
        <div className="mx-3 mb-3 p-3 bg-gradient-to-br from-wellnet-50 to-amber-50 rounded-2xl border border-wellnet-100">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <div className="text-xs font-semibold text-wellnet-700">Kuriftu × Well-Net</div>
          </div>
      ...
      {/* Dimmed Overlay Interface for Small Screen Viewports */}
      {sidebarOpen && (
        <div
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <header className="lg:hidden sticky top-0 z-30 flex items-center gap-3 px-4 py-3 bg-white border-b border-gray-100">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-sm text-gray-900">Well-Net</span>
            </div>
            {(profile?.current_gut_score ?? 0) > 0 && (
              <div className="ml-auto flex items-center gap-1 bg-wellnet-50 rounded-full px-2.5 py-1">
                ...
                <span className="text-[10px] text-wellnet-500">/100</span>
              </div>
            )}
          </header>

          {/* Core App Stage and Ad Sidebar Integration Container */}
          {/* Updated: Upgraded max boundary from max-w-7xl to max-w-screen-2xl */}
          <div className="flex flex-1 gap-6 max-w-screen-2xl mx-auto w-full px-4 lg:px-6 py-4 lg:py-6 items-start">
