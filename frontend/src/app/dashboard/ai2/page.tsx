              ...
              <>
                ...
                <div id="meal-plan-print" ref={printRef}>
                  ...
                  {(plan.days || []).map((day: any) => (
                    <div key={day.day} className="card mb-3" style={{breakInside:"avoid" as any}}>
                      {/* Day header */}
                      <div className="flex items-center justify-between mb-3 pb-2 border-b border-gray-100">
                        ...
                        {day.is_fasting_day && (
                          <span className="badge-amber text-[10px]">✦ Fasting day</span>
                        )}
                      </div>
                      ...
                      {(["breakfast","lunch","dinner"] as const).map(mealKey => {
                        ...
                        const note: string = meal?.note || meal?.notes || ""
                        return (
                          <div key={mealKey} className="mb-3 last:mb-0">
                            <div className="flex items-center gap-1.5 mb-1.5">
                              <span className="text-sm">{MEAL_EMOJI[mealKey]}</span>
                              <span className="text-xs font-semibold text-gray-500 capitalize">{mealKey}</span>
                            </div>
                            {/* Each food as its own chip — not comma-separated */}
                            <div className="flex flex-wrap gap-1.5 mb-1">
                              {foods.length > 0 ? foods.map((food, i) => (
                                <span key={i} className="inline-flex items-center px-2.5 py-1 bg-wellnet-50 text-wellnet-800 text-xs font-medium rounded-full border border-wellnet-100">
                                  {food}
                                </span>
                              )) : (
                            ...
                            {/* One-sentence note on how they eat together */}
                            {note && (
                              <p className="text-[11px] text-gray-400 italic leading-relaxed pl-0.5">
                                {note}
                              </p>
                            )}
                          </div>
